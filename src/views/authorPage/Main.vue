<template>
  <div class="namePage">
    <!-- 页面头部 -->
    <pageHead pageNum="3" :isPhone="isPhone"> </pageHead>
    <div class="body" :class="{ phone_body: isPhone }">
      <div class="title" :class="{ phone_title: isPhone }">
        ...
        >
          ...
          />
          <!-- 搜索框 -->
          <searchModule @on-search="search" :isPhone="isPhone" page="author">
          </searchModule>
          <!-- 底部边框 -->
      ...
      <div class="works">
        ...
        <div class="excellent_div" :class="{ phone_excellent_div: isPhone }">
          ...
          >
            <showBox :isPhone="isPhone" :info="item" right="0"> </showBox>
          </div>
        </div>
  ...
  methods: {
    ...
    searchWorks() {
      ...
      this.getWorksInfo(param).then((item) => {
        this.pageSize = this.switchPageNum(item.authNum);
